...
|-----------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **angular-developer**       | Gera código Angular e orienta arquitetura em toda a stack: componentes, reatividade (signals, `linkedSignal`, `resource`, `effect`), forms (signal/reactive/template), DI, routing, SSR/rendering, acessibilidade (Angular Aria), animações, estilização (Tailwind), testes e CLI/migrações. Roda `ng build` ao final. | Ao **criar projeto/componente/serviço** ou para best practices amplas (DI, routing/guards, SSR, ARIA, animações, CLI, migrations). Para regras específicas da casa: layout → `angular-folder-structure`, HTTP → `angular-http`, padrões de componente/TS → `angular-patterns`/`typescript-patterns`. |
...
| **symlinks**                | Cria, repara e verifica links simbólicos dos itens de `.agents/skills` para outras pastas de agentes locais (`.claude/skills`, `.devin/skills`), mantendo uma única fonte de verdade sem copiar arquivos.                                                                          | Ao **sincronizar, linkar ou espelhar** as mesmas skills entre ferramentas (Codex/Claude/Devin), ou verificar/reparar links quebrados. Em conflito (arquivo/dir real ou link para outro lugar), para e pede confirmação. Não copia arquivos. |
