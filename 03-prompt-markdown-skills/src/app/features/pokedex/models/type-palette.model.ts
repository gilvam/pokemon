...
/**
 * Accessible color palette per Pokémon type. Backgrounds are light, desaturated
 * tints paired with a single dark text color so every combination clears WCAG AA
 * (contrast >= 4.5:1) — verified in the spec. Card backgrounds blend the (up to two)
 * type tints into a soft gradient.
 */
export class TypePalette {
  private static readonly text = '#1f2937';
  private static readonly neutralBackground = 'var(--mat-sys-surface-container-high)';
  ...
  /** CSS `background` value for a card given its 0, 1 or 2 types. */
